...
- `_evaluate_log_intensity(fn, marks, n, spatial_shape)`:
  - calls `fn(marks)` and copies the result to a new float64 array
  - checks it has shape `(n, *spatial_shape)`, with no NaN or `+inf`
