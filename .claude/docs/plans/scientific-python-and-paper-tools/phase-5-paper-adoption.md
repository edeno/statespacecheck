...
6. **Regenerate and compare** against the baseline from task 1:
   ...
   - Diff the Figure 3/4 summary JSONs. They must be identical apart from provenance, because `event_diagnostics` is unchanged, with one exception: phase 3b scaled the p-value tie tolerance by each event's own largest predictive mark probability instead of the batch's. That can change a p-value only at a near-exact tie (mark probabilities within about `16 * eps * n_bins` of each other); if any reported value differs, find the events and report them to the user before accepting it. The allowed provenance changes are `source.statespacecheck_version`, `source.source_tree_sha256` and `source.uv_lock_sha256` in both, plus `figure04_decode_cache.statespacecheck_version` and `figure04_decode_cache.diagnostics_fingerprint_sha256`.
   ...
   - Decide [overview Open Question 5](overview.md#open-questions) (baseline quantiles per spike versus over the dense matrix) with the user; if the answer is per spike, change `compute_baseline_diagnostic_thresholds` here and report the threshold and reported-value changes.
