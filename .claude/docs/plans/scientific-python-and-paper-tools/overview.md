...
5. **Should the paper's baseline thresholds pool per spike?** Found in the phase 3b review. The package's docs and `flag_events` take baseline quantiles over the per-spike values. The paper's `compute_baseline_diagnostic_thresholds` (`statespacecheck-paper/src/statespacecheck_paper/diagnostics.py`, `_threshold`) takes them over the dense `(n_time, n_cells)` matrix, which `compute_spike_event_diagnostics_from_rates` fills with `matrix[spike_time_ind, spike_cell_ind] = values`: when a unit fires twice in one time bin, the later spike's value overwrites the earlier one before the quantile. Current answer: pass the per-event arrays (`event_hpd_overlap`, `event_kl_divergence`, restricted to baseline events) in phase 5, after measuring how many baseline spikes share a (time bin, unit) in the Figure 3 simulation and whether the thresholds and reported values change. The paper's numbers are the user's call; ask before regenerating them.
6. **Clusterless decoders beyond a single KDE encoding model.** Found in the phase 4a review with a fitted `non_local_detector` model. The KDE backend works through an adapter, documented and checked in `docs/decoders.md` (electrode as part of the mark; log intensity from the fitted parameters, not NLD's floored helpers). Two cases are not handled: the GMM backend, whose separately fitted ground intensity need not be the integral of its joint model, and detectors with several observation models (local states, several encoding groups or environments, a no-spike state), where summing the predictive distribution over states and applying one mark model is wrong. Current answer: out of scope for 0.3.0; plan them as their own work if needed.
