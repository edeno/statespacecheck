...
- **Breaking:** `flag_extreme_kl()` always flags infinite KL divergence (disjoint supports), which it used to ignore. `z_thresh` and `min_len` are keyword-only.
...
- `kl_divergence()`, `hpd_overlap()`, `highest_density_region()`, `predictive_density()` and `log_predictive_density()` process time in chunks, cutting peak memory by 10–20× (for 100,000 time bins × 500 positions, from 1.3–4.5 GB to 0.16–0.37 GB) at the same speed. Results are unchanged.
- `import statespacecheck` takes about 0.1 s instead of 0.4 s: it no longer imports `matplotlib.pyplot` (`plot_diagnostics()` imports it when called) or `scipy.stats`. Results are unchanged.
