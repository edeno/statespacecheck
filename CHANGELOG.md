...
- `monte_carlo_mark_pvalue()` in the new `statespacecheck.continuous_marks` module: the rank-based predictive p-value estimated by Monte Carlo, for marks that cannot be enumerated (clusterless waveform features). It returns a `MarkPredictiveCheck`; the model is given as a `LogMarkIntensity` and a `MarkSampler` callable (the intensity as its log, since densities of many-feature marks underflow). The README shows the paper's Figure 2 example.
