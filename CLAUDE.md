...
- **`continuous_marks.py`**: the predictive p-value by Monte Carlo for marks that cannot be enumerated (clusterless waveform features): `monte_carlo_mark_pvalue`, its `MarkPredictiveCheck` result, and the `LogMarkIntensity`/`MarkSampler` callable types (the intensity is passed as its log: many-feature mark densities underflow). States are drawn from `event_weighted_predictive` (in `events.py`).
